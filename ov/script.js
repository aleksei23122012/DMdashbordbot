...
async function renderNachpoz(categoryColumn) {
  ...
  mainBlock.classList.remove('is-centered-message');
...
async function renderTupoz(category, type) {
  ...
  mainBlock.innerHTML = createAccordionHtml(data, category, false, false);
...
function createAccordionHtml(items, category, isSearchResult = false, autoExpandSingleItem = false) {
    if (!items || items.length === 0) {
        ...
        mainBlock.classList.add('is-centered-message'); // Добавляем класс для центрирования
        return `<p>${message}</p>`;
    }
    mainBlock.classList.remove('is-centered-message'); // Убираем класс, если есть результаты
...
function setupNachpozListeners() {
  mainBlock.querySelectorAll('.button-grid button').forEach(button => {
      button.addEventListener('click', (e) => renderTupoz(currentCategory, e.target.textContent));
  });
}
