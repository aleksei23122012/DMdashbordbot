document.addEventListener('DOMContentLoaded', () => {
    ...
    authSubmitBtn.addEventListener('click', async () => {
        ...
        try {
            // ===============================================================================
            // ИЗМЕНЕНИЕ ЛОГИКИ: Ищем в persinfo по id_mango, но забираем связанный с ним tg username
            // ===============================================================================
            const { data, error } = await supabaseClient
                .from('persinfo')
                .select('tg') // ЗАПРАШИВАЕМ КОЛОНКУ 'tg'
                .eq('id_mango', inputId)
                .maybeSingle();

            if (error) throw error;

            // Проверяем, что пользователь найден И у него есть связанный tg username
            if (data && data.tg) {
                // Пользователь найден, используем его TG username для дальнейшей работы
                userIdentifier = data.tg; // ИСПОЛЬЗУЕМ ПОЛУЧЕННЫЙ TG USERNAME
                userIdentifierColumn = 'tg'; // УКАЗЫВАЕМ, ЧТО ИСКАТЬ НУЖНО В КОЛОНКЕ 'tg'
                ...
                authError.textContent = 'ID не найден или с ним не связан аккаунт Telegram.';
            }
        } catch (err) {
    ...
    function initializeDashboard() {
        console.log(`Инициализация дашборда для пользователя: ${userIdentifier} (по колонке ${userIdentifierColumn})`);
    ...
    function applyRelativeGradient(selector, isReversed = false) {
        const cells = document.querySelectorAll(selector); if (cells.length === 0) return;
        const cellData = Array.from(cells).map(cell => { const text = cell.textContent; const value = text.includes(':') ? timeToSeconds(text) : parseFloat(text.replace('%', '')); return { cell, value }; });
        const values = cellData.map(item => item.value).filter(v => !isNaN(v)); if (values.length === 0) return;
        const min = Math.min(...values); const max = Math.max(...values);
        if (min === max) { cellData.forEach(item => { const hue = item.value === 0 ? 0 : 60; item.cell.style.backgroundColor = `hsl(${hue}, 90%, 85%)`; item.cell.style.color = `hsl(${hue}, 80%, 25%)`; item.cell.style.fontWeight = '600'; item.cell.style.borderRadius = '4px'; }); return; }
        const range = max - min;
        cellData.forEach(item => {
            let normalized = 0.5; if (range > 0 && !isNaN(item.value)) { normalized = (item.value - min) / range; }
            if (isReversed) { normalized = 1 - normalized; }
            const hue = normalized * 120;
            item.cell.style.backgroundColor = `hsl(${hue}, 90%, 85%)`; item.cell.style.color = `hsl(${hue}, 80%, 25%)`; item.cell.style.fontWeight = '600'; item.cell.style.borderRadius = '4px';
            if (item.value === 0) { item.cell.style.backgroundColor = 'hsl(0, 90%, 85%)'; item.cell.style.color = 'hsl(0, 80%, 25%)'; }
        });
    }
    ...
    // --- ОСНОВНЫЕ ФУНКЦИИ ЗАГРУЗКИ (без изменений, т.к. они универсальны) ---
    async function fetchDashboardData(period) {
        if (!userIdentifier || !userIdentifierColumn) return;

        const userTable = period === 'day' ? 'TMday' : 'TMmonth';
        const avgTable = period === 'day' ? 'AVGday' : 'AVGmonth';
        
        console.log(`Запрос данных для ${userIdentifierColumn} = ${userIdentifier} из таблицы ${userTable}`);
        ...
        if (!userData) { alert(`Данные для пользователя "${userIdentifier}" не найдены в таблице ${userTable}. Убедитесь, что колонка ${userIdentifierColumn} заполнена правильно.`); return; }
    ...
    async function fetchPositionData() {
        ...
        if (currentUserPosition === 'РГТМ') { filterColumn = 'team'; filterValue = currentUserTeam; headerText = currentUserTeam; } else { filterColumn = 'dolg'; filterValue = currentUserPosition; headerText = currentUserPosition; }
    ...
    async function fetchDynamicsData() {
        ...
        const now = new Date();
        for (let i = 5; i >= 0; i--) { const date = new Date(now.getFullYear(), now.getMonth() - i, 1); labels.push(date.toLocaleString('ru-RU', { month: 'short' }).replace('.', '')); }
        for (let i = 1; i <= 6; i++) { yourTraffic.push(timeToHours(data[`${i}trafic`])); avgTraffic.push(timeToHours(data[`${i}avg_trafic`])); yourLeads.push(data[`${i}lid`] || 0); avgLeads.push(data[`${i}avg_lid`] || 0); yourTime.push(timeToSeconds(data[`${i}time`])); avgTime.push(timeToSeconds(data[`${i}avg_time`])); }
    ...
    function switchView(viewId, buttonToActivate, task) {
        ...
        document.querySelectorAll('.header-buttons button').forEach(button => button.classList.remove('active'));
